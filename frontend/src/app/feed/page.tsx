                ...
                <div className="px-4 pt-4 pb-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <h3 className="text-[14px] font-bold text-gray-900 truncate">{course.title}</h3>
                      <p className="text-[12px] text-gray-400 mt-0.5">
                        {course.creator_name ?? '익명'} · {formatDate(course.created_at)}
                      </p>
                    </div>
                    <button
                      onClick={() => handleDownload(course.id)}
                      disabled={downloading === course.id || downloaded.has(course.id)}
                      className={`shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-xl text-[12px] font-bold transition-colors ${
                        downloaded.has(course.id)
                          ? 'bg-emerald-50 text-emerald-600'
                          : 'bg-blue-50 text-blue-600 hover:bg-blue-100 active:bg-blue-200 disabled:opacity-50'
                      }`}
                    >
